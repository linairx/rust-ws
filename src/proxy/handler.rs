...
use crate::proxy::{parse_shadowsocks, parse_trojan, parse_vless};
use crate::AppState;
...
async fn handle_websocket(socket: WebSocket, _state: Arc<AppState>) {
    ...
    // VLESS and Trojan have already consumed the header, but for Shadowsocks
    // the entire first packet contains encrypted payload that needs to be forwarded
